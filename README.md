...
server or in CI against the PR's real Vercel preview, at two viewports —
`desktop` (1440x900) and `mobile` (390x844), either side of the site's own
`1024px` layout breakpoint. See
[docs/testing-pyramid.md](docs/testing-pyramid.md) for what each layer
catches, a test-by-test e2e coverage breakdown, and a diagram of where e2e
sits in the CI/deploy pipeline.
