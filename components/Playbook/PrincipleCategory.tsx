...
export function PrincipleCategory({ category, isExpanded, onToggle }: PrincipleCategoryProps) {
  return (
    <div className="rounded-lg border border-gray-200 bg-white">
      {/* Header */}
      <button
        onClick={onToggle}
        className="w-full rounded-t-lg p-4 text-left transition-colors hover:bg-gray-50"
          ...
          >
            <div className="space-y-3 p-4">
              {category.principles.map((principle, idx) => (
                <div key={idx}>
                  <p className="font-medium text-gray-900">{principle.title}</p>
