...
function findSkill(
  ...
  skill: { title: string; icon?: string } | undefined;
  category: { title: string; items: { title: string }[] };
} | null {
  const skillsData = data as {
    skills?: Array<{ title: string; items: Array<{ title: string; icon?: string }> }>;
  } | null;
...
export function SkillCard({ skillName, category, onClose }: SkillCardProps) {
  ...
  if (!skillData?.skill) {
    return (
      <div className="rounded-lg border border-gray-200 bg-white p-4">
        <p className="text-sm text-gray-600">Skill data not available</p>
      </div>
    );
  }
  ...
  return (
    <article className="rounded-lg border border-blue-200 bg-blue-50 p-4">
      ...
      <button
        onClick={onClose}
        className="mb-2 text-sm font-medium text-blue-700 hover:text-blue-900 md:hidden"
      ...
      {skill.icon && <div className="mb-4 text-center text-5xl">{skill.icon}</div>}
      ...
      <div className="space-y-2 rounded-lg bg-white p-3">
        ...
        <div className="flex flex-wrap gap-2">
          {(categoryData.items || []).slice(0, 3).map((relatedSkill, idx) => (
            <span key={idx} className="rounded-full bg-gray-200 px-2 py-1 text-xs text-gray-800">
              {relatedSkill.title}
            </span>
          ))}
        </div>
