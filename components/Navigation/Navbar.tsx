...
export function Navbar() {
  ...
  const { navbar: navData } = navbar.data;
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-2 font-bold text-gray-900 dark:text-white">
            {navData.logo && <span className="text-2xl">{navData.logo}</span>}
            <span className="hidden sm:inline">{navData.brandName || 'Prannoy'}</span>
          </Link>

          {/* Desktop Navigation */}
          <div className="hidden items-center gap-1 md:flex">
            {navData.navigationSections &&
              navData.navigationSections.map((section) => (
